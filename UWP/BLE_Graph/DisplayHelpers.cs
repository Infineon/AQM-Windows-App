...
namespace BLE_Graph
{
    public static class BLE_ErrorCodes
    {
        #region Error Codes
        public readonly static int E_BLUETOOTH_ATT_WRITE_NOT_PERMITTED = unchecked((int)0x80650003);
        public readonly static int E_BLUETOOTH_ATT_INVALID_PDU = unchecked((int)0x80650004);
        public readonly static int E_ACCESSDENIED = unchecked((int)0x80070005);
        public readonly static int E_DEVICE_NOT_AVAILABLE = unchecked((int)0x800710df); // HRESULT_FROM_WIN32(ERROR_DEVICE_NOT_AVAILABLE)
        #endregion
    }
    ...
    public static class DisplayHelpers
    {
        ...
        public static async void ListAllBleServicesAndCharacteristics(GattDeviceServicesResult GattServicesResult) {
            ...
            foreach (var service in services) {
                Debug.WriteLine("\t{0}, {1}", service.Uuid, DisplayHelpers.GetServiceName(service));
                ...
                try {
                    ...
                    if (accessStatus == DeviceAccessStatus.Allowed) {
                        ...
                        if (resultC.Status == GattCommunicationStatus.Success) {
                            characteristics = resultC.Characteristics;
                        }
                        else {
                ...
                foreach (GattCharacteristic c in characteristics) {
                    Debug.WriteLine("\t\tChr: " + c.Uuid.ToString() + ", " + c.UserDescription.ToString() + ".");
                    //_characteristicList.Add(c);

                    var descriptors1 = await c.GetDescriptorsAsync();
                    if (descriptors1.Status == GattCommunicationStatus.Success && descriptors1.Descriptors.Count >= 1) {
                        //Debug.WriteLine("\t\t\tGD: {0}", descriptors1.Descriptors);

                        // Get Descriptor and parse its content
                        var dsc = descriptors1.Descriptors[0];
                        var ReadValue = await dsc.ReadValueAsync();
                        // iBuffer to byte array
                        byte[] data;
                        CryptographicBuffer.CopyToByteArray(ReadValue.Value, out data);
                        // UTF conversion - String from bytes  
                        var utfString = Encoding.UTF8.GetString(data, 0, data.Length);
                        if (!string.IsNullOrWhiteSpace(utfString) && dsc.Uuid != null) {
                            Debug.WriteLine("\t\t\tDes: " + utfString + ", " + dsc.Uuid.ToString());
                        }
                    }
                    else {
                        Debug.WriteLine($"\t\t\tGetDescriptorsAsync failed {0}", descriptors1.Status);
                    }
    ...
    public class BluetoothLEDeviceDisplay : INotifyPropertyChanged
    {
        ...
        public string Address => Regex.Replace(DeviceInformation.Id, @"^.*(?=([\d\w]{2}:){5}[\d\w]{2}$)", "");
